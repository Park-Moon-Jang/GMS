	...
	function scrapSel(curPage){
		$.ajax({
			...
			success: function(data){
				...
				str += '<th><input type="checkbox" id="chekAll" name="chekAll"></th>';
				str += '<th>이미지</th><th>상품이름</th><th>브랜드</th><th>수량</th><th>가격</th><th>비고</th></tr>';
				
				$.each(data.IList, function(i){
					str += "<tr id='"+i+"'>"
					str += '<td><input type="checkbox" name="'+data.IList[i].item_No+'"></td>'
					str += '<td id="img"></td>'
					...
					str += '<td><button onclick="delete("'+data.IList[i].item_No+'")">삭제</button></td>';
					str += "</tr>"
...
$(document).on("click",".paging",function(){
	...
	<div id="content" align="center">
		<table id="scrapPaging">
			<tr>
				<th><input type="checkbox" id="chekAll" name="chekAll"></th>
				<th>이미지</th>
				<th>상품이름</th>
				<th>브랜드</th>
				<th>수량</th>
