...
<body>
<style>
	#comentSubmit{width:100;height:70px;}
</style>
	<script>
		$(document).ready(function() {
					...
					function selectComent(curPage){
						...
						success : function(data) {
								var str = '<table id="comentTable"><tr><th colspan="3" align="left">댓글'+data.totalCount+'개</th></tr>';
							$.each(data.CList, function(i) {
								str += "<tr id='"+i+"'>"
								str += "<td>"+data.CList[i].user_NickName+"</td>";
								str += '<td>'+moment(data.CList[i].write_Date).format("YYYY-MM-DD. hh:mm")+'</td>'
								str +='<td>';
								...
								str += '</td></tr><tr>'
								str += '<td colspan="3"  height="70" width="500">'+data.CList[i].content+'</td></tr>';
						...
						error : function(jqXHR, Status, error) {
							console.log("selectComent Error!");
						}
					});
				}
						...
						$.ajax({
							...
							success : function(data) {
								...
								$.each(data, function(i) {
									str = '<td id="brand">'
											+ data[i].company_Name + '</td>';
									$("#brand").html(str);
									str = '<td id="category">'
											+ data[i].category_Name + '</td>';
									$("#category").html(str);
									str = '<td id="item_Name">'
											+ data[i].item_Name + '</td>';
									$("#item_Name").html(str);
									str = '<td id="amount">' + data[i].amount
											+ '</td>';
									$("#amount").html(str);
									str = '<td id="price">' + data[i].price
											+ '</td>';
									$("#price").html(str);
								})
							},
					...
					$(document).on("click","#deleteComent",function(){
						var coment_No = $("#coment_No").val()
						$.ajax({
							type : "POST",
							url : "/app/user/deleteComent",
							data : {"coment_No":coment_No},
							success : function(data) {
								alert("댓글 삭제");
								selectComent(1);
							},
							error : function(jqXHR, Status, error) {
								console.log("deleteComent Error!");
							}
						});
					});
				})
	...
	<div id="content" align="center">
		...
		<table>
			...
			<tr>
				<td colspan="2" rowspan="6" id="itemImg"><img src=""
